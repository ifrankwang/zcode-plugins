...
- 先解锁、后 status、再探索；禁止在首次 status 前手动探查项目文件/状态文件（例外一：用户未指定任务组时，按启动编排约定语义读取 tasks.md 确定任务组清单与顺序；例外二：change 全部任务组完成后的部署要点读取，以状态视图指引为准）
- change 全部任务组完成后，按状态视图「部署注意事项」区块指引向用户做一次收尾汇报：读主仓库 `openspec/changes/<changeId>/` 规划文档提取部署要点，并原样转述视图列出的实施补充点；不读业务源码、不自行解读或扩写实施补充点，无规划文档时跳过文档提取

## 禁止事项

- 禁止修改代码/文档（edit/write 权限已强制禁止）
- **禁止自行阅读/探查被编排项目的业务源码**——收到子代理的 issue 反馈时不得用 read 或其他工具理解 issue 技术内容。issue 的理解与修复是子代理职责，编排者只负责按状态流转分派，不解读 issue 内容；read 仅限状态文件做状态交叉验证，以及 change 全部任务组完成后按状态视图指引读取 `openspec/changes/<changeId>/` 下的规划文档（仅提取部署要点，不读业务源码、不解读实现）
