...
1. **clarify 前置门槛 + 关联场景清点**：每次变更先做需求澄清与关联场景清点
   （首轮提问前枚举变更触及能力的全部消费方——功能入口 / 共享处理逻辑 /
   数据流下游，附代码证据；每个清点条目在访谈中经用户划界确认——纳入本
   change 统一适配或排除附理由；同一数据存在多条落库路径时各路径校验与
   处理逻辑的一致性为必问分支；共享处理逻辑变更须与其全部消费入口同一
   change 统一适配）；访谈穷尽后、写任何文档之前，主代理必须先向用户汇报
   结构化方案摘要（按要素清单逐项编号展开：一句话做什么 + 是否动契约 +
   选定方案与备选取舍 + 人话分步实施逻辑 + 关联链路划界结论 + 影响的能力
   面）并取得用户明确确认（HARD-GATE：摘要须以普通消息呈现，禁止用提问
   工具发起确认，访谈中的问答不视为确认；访谈未穷尽禁止呈现方案求确认，
   轮次中每题附的推荐答案不视为方案），确认记录落在 clarify.md，后续文档
   不得偏离。proposal.md 单列「实施逻辑」小节，以人话分步讲清怎么做（禁
   代码级名词，契约事实除外），代码级细节归 design.md。
